...
const ArchitectureOverview: React.FC<ArchitectureOverviewProps> = ({ 
  ...
  return (
    <div className="h-full flex flex-col space-y-12 pb-20 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {/* Модуль Базы Данных */}
        <div className="bg-slate-900/30 border border-slate-800 rounded-[3rem] p-10 flex flex-col justify-between shadow-xl">
          <div className="mb-8">
            <div className="w-12 h-12 bg-emerald-500/10 rounded-2xl flex items-center justify-center text-emerald-500 mb-4 border border-emerald-500/20">
            ...
            <h3 className="text-xl font-black text-white mb-2">Облачный Бэкап</h3>
            <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest leading-relaxed">
              Экспортируйте проект в JSON для сохранения на диске. Это единственный способ сохранить данные при смене браузера или хостинга.
            </p>
          </div>
          <div className="flex gap-2">
            <button 
              onClick={handleExportDB}
              className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-2xl text-[10px] uppercase transition-all shadow-lg shadow-emerald-600/20"
            >
              Экспорт БД
            </button>
            <button 
              onClick={() => fileInputRef.current?.click()}
              className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-400 font-black py-4 rounded-2xl text-[10px] uppercase transition-all"
            >
              Импорт БД
            </button>
        ...
        {project.sections.map(section => (
          ...
          >
            <div className="w-14 h-14 rounded-2xl bg-primary-500/10 flex items-center justify-center text-primary-500 mb-8 group-hover:bg-primary-500 group-hover:text-white transition-colors duration-300">
